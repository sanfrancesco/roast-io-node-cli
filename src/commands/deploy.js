...
function withPath (config, cmd) {
  ...
  if (path) {
    ...
    return inquirer.prompt({name: 'path', message: 'Path to deploy? (current dir)'}).then(function (result) {
...
exports.cmd = function (config, cmd) {
  ...
  if (siteId) {
    ...
    sitePromise = inquirer.prompt([{name: 'confirm', type: 'confirm', message: 'No site id specified, create a new site'}]).then(function (result) {
