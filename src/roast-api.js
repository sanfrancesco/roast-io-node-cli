...
module.exports = class RoastApi {
  ...
  withAuthorization() {
    if (!this.isAuthorized())
      return Promise.reject(
        "Not authorized: Instantiate client with access_token"
      );
  ...
  request(options) {
    const client = this;
    const path = options.raw_path
      ? options.url
      : "/api/" + this.VERSION + options.url;
    const headers = options.headers || {};
    const retries = options.retries
      ? options.retries
      : options.retries === 0 ? 0 : 3;
    let body = null;
  ...
  collection(options, meta = {}) {
    ...
    if (meta.page) {
      params.push(["page", meta.page].join("="));
    }
    if (meta.per_page) {
      params.push(["per_page", meta.per_page].join("="));
    }
      ...
      return client
        ...
        .then(function(response) {
          const result = response.data.map(
            d => new options.model(response.client, d)
          );
