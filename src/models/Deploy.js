...
export default class Deploy extends Model {
  ...
  waitForReady () {
    if (this.isReady()) {
      ...
      return delay(1000).then(this.refresh.bind(this)).then(this.waitForReady.bind(this));
    }
  }
  waitForPreparation () {
    if (this.isPrepared()) {
      ...
      return delay(1000).then(this.refresh.bind(this)).then(this.waitForPreparation.bind(this));
    }
  }
  uploadFiles (files, progress) {
    if (!(this.state == 'uploading' || this.state == 'prepared')) return Promise.resolve(this);
